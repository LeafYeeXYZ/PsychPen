...
注: 本文档专为PsychPen的AI助手撰写, 将省略AI助手可以通过函数调用完成的功能; 如果用户希望了解PsychPen的所有功能、查看PsychPen的学术引用信息, 请告诉用户完整文档地址: <https://github.com/LeafYeeXYZ/PsychPen>

## 1 项目简介

PsychPen 是一个AI加持的在线统计分析和数据可视化软件, 有 `数据`、`变量`、`绘图`、`统计`、`工具` 五大主要功能模块, 以及 `AI辅助分析` 和 `R语言服务器` 两大独立辅助模块. 相比于传统的心理学统计软件, PsychPen 具有以下优势:
...
由于 `JavaScript` 的相关统计库较少, 作者基于 `Docker` 和 `R` 开发了一个简单的服务端, 用于为 PsychPen 提供执行 `R` 脚本的接口.
