...
- [PsychPen 使用文档](#psychpen-使用文档)
  ...
  - [5 统计视图](#5-统计视图)
    ...
    - [5.5 相关](#55-相关)
      ...
      - [5.5.2 Bartlett 球形度检验](#552-bartlett-球形度检验)
      - [5.5.3 KMO 测试](#553-kmo-测试)
    - [5.6 回归](#56-回归)
      - [5.6.1 一元线性回归](#561-一元线性回归)
      - [5.6.2 二元线性回归](#562-二元线性回归)
    - [5.7 信度分析](#57-信度分析)
      - [5.7.1 重测信度/复本信度](#571-重测信度复本信度)
      - [5.7.2 分半信度](#572-分半信度)
      - [5.7.3 同质性信度](#573-同质性信度)
    - [5.8 中介效应分析](#58-中介效应分析)
      - [5.8.1 简单中介效应检验](#581-简单中介效应检验)
  - [6 工具视图](#6-工具视图)
...
注意: PsychPen 计算众数的方式是: 如果有多个众数, 则显示皮尔逊经验公式计算的众数 (中位数 * 3 - 均值 * 2)
...
单因素方差分析用于检验一个有多个水平的被试间自变量, 对一个因变量的影响是否有显著差异. 在单因素方差分析页面中, 你可以选择你要进行检验的数据变量和分组变量, 点击 `计算` 按钮即可进行单因素方差分析. 除此之外, 你还可以选择进行事后检验的方法 (Scheffe、Bonferroni)
...
#### 5.5.2 Bartlett 球形度检验

Bartlett 球形度检验用于检验多个变量之间的相关性. 在 Bartlett 球形度检验页面中, 你可以选择你要进行检验的变量 (至少选择两个), 点击 `计算` 按钮即可进行 Bartlett 球形度检验

结果会给出你选择的变量之间的 Bartlett 球形度检验的统计量 (卡方) 和显著性水平, 并输出一个相关矩阵

通常, 只有在变量之间的相关性较强时 (即结果显著时), 才会进行因子分析等进一步的分析

![](readme/stat-24.png)

#### 5.5.3 KMO 测试

KMO 测试也是用于检验多个变量之间的相关性. 在 KMO 测试页面中, 你可以选择你要进行检验的变量 (至少选择两个), 点击 `计算` 按钮即可进行 KMO 测试

KMO 测试的结果是一个 0-1 之间的值, PsychPen 除了会给出 KMO 值, 还会输出一个相关矩阵和一个偏相关矩阵

通常, KMO 值大于 0.7 时, 进行进一步的因子分析效果才会较好
