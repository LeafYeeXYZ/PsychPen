...
> 注意: 以下变量处理的内部顺序是: `定义缺失值` -> `缺失值插值` -> `标准化/中心化/离散化` -> `数据筛选` -> `描述统计`; 而 `计算变量` 是一个独立的操作, 不受这个顺序的影响
...
// 手动进行标准化（仅为演示，实际使用时推荐直接使用标准化/中心化/离散化功能）
( :::题目1::: - mean(:::题目1:::) ) / std(:::题目1:::)
```

#### 3.6.2 表达式条件判断

计算变量还可以通过条件判断来给数据打标签或将连续变量转换为等级变量; `条件 ? 条件满足时的值 : 条件不满足时的值` 称为三元运算符, 用于简单的条件判断
