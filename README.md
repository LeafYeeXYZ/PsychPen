...
| 导入前 | 导入后 |
| :---: | :---: |
| ![](readme/data-4.png) | ![](readme/data-5.png) |
...
此外, 你还可以词云图的颜色、词云图的形状、单词最大和最小尺寸、单词旋转角度、手动关闭词语切分等
...
因子分析前提检验用于检验数据是否适合进行因子分析, 包括 Bartlett 球形度检验和 KMO 测度. 在因子分析前提检验页面中, 你可以选择你要进行检验的变量 (至少选择两个), 点击 `计算` 按钮即可进行因子分析前提检验. 除此之外, 你还可以选择显示相关矩阵和偏相关矩阵

Bartlett 球形度检验的原假设是"变量之间没有相关性" (即相关矩阵是单位矩阵), 当结果显著时, 可以进行因子分析; KMO 测度的值在 0-1 之间, 值越大, 变量之间的相关性越强, 因子分析效果越好, 通常 KMO 值大于 0.7 时, 才适合进行因子分析
