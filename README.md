...
在心理学和教育学研究中, 通常需要对原始数据进行一些计算, 以得到最终用于分析的变量. 在变量视图中, 你可以点击 `计算变量` 按钮, 在此页面输入计算表达式 (见 [3.7 表达式语法](#37-表达式语法)) 和新变量名, 点击 `计算` 按钮并确认即可生成新变量. 生成后, 你可以在 `变量表格` 和 `数据视图` 中看到生成的新变量
