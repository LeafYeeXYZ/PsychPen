import { useNav, PLOTS_SUB_PAGES_MAP, type PLOTS_SUB_PAGES_LABELS } from '../lib/useNav'
import { Cascader } from 'antd'

export function PlotsView() {

  const { activePlotsViewSubPage, setPlotsViewSubPage, plotsViewSubPage } = useNav()
      ...
      <div className='flex flex-col justify-start items-center w-full h-full p-4'>
        ...
        <div className='w-full flex justify-start items-center gap-3 mb-4'>
          <Cascader
            placeholder='请选择绘图类型'
            defaultValue={[
              Object.entries(PLOTS_SUB_PAGES_MAP).find(([, value]) => value.includes(activePlotsViewSubPage))![0],
              activePlotsViewSubPage
            ]}
            options={Object.entries(PLOTS_SUB_PAGES_MAP).map(([key, value]) => ({
              value: key,
              label: key,
