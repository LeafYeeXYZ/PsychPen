...
export function twoSampleTTestCalculator(config: {
	dataVar: string
	groupVar: string
	expect: number
	twoside: boolean
...
对被试间变量"${dataVar}" (分组变量: "${groupVar}") 进行${twoside ? '双尾' : '单尾'}独立样本T检验 (Student's T Test). 原假设 (H<sub>0</sub>) 为"均值差异 = ${expect}"; 显著性水平 (α) 为 ${alpha}.
