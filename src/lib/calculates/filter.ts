  ...
  constructor(
    ...
    for (const col of dataCols) {
      if (col.filterMethod === '等于' && col.filterValue?.length) {
        dataRows = dataRows.filter((row) => col.filterValue!.some((value) => row[col.name] == value)) // 故意使用 == 而非 ===
        continue
      }
      if (col.filterMethod === '不等于' && col.filterValue?.length) {
        dataRows = dataRows.filter((row) => col.filterValue!.every((value) => row[col.name] != value)) // 故意使用 != 而非 !==
        continue
      }
      ...
      if (col.filterMethod === '大于' && col.filterValue?.length === 1) {
        dataRows = dataRows.filter((row) => Number(row[col.name]) > Number(col.filterValue![0]))
        continue
      }
      ...
      if (col.filterMethod === '区间' && col.filterRange?.length === 2) {
        dataRows = dataRows.filter((row) => Number(row[col.name]) >= Number(col.filterRange![0]) && Number(row[col.name]) <= Number(col.filterRange![1]))
        continue
      }
      if (col.filterMethod === '高于平均值') {
        const avg = mean(dataRows.filter((row) => row[col.name] !== undefined).map((row) => Number(row[col.name])))
      ...
      if (col.filterMethod === '高于中位数') {
        const med = median(dataRows.filter((row) => row[col.name] !== undefined).map((row) => Number(row[col.name])))
