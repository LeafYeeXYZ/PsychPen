import { z } from 'zod'
import { STATISTICS_SUB_PAGES_LABELS } from '../../../hooks/useNav.tsx'
...
> = new Map([
	...
	[
		...
		'如果各个题目是测量同一心理特质, 则各个题目得分间相关越高则信度越好; 同质性信度即是各个题目与总分的相关系数, 常用 alpha 系数表示 (应用中, alpha 的值至少要大于0.5, 最好能大于0.7), 近期也有越来越多的研究使用 omega 系数. 在本页面中, 可以选择量表的所有题目, 点击 `计算` 按钮即可进行同质性信度检验, 可以同时计算 alpha 系数和 omega 系数. 还可以选择分组变量, 以便对不同分组的数据进行信度检验.',
	],
	[
