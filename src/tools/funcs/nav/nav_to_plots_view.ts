...
export const nav_to_plots_view_desc: Map<PLOTS_SUB_PAGES_LABELS, string> =
	new Map([
		...
		[
			...
			'选择要进行绘图的变量的类型 (被试内变量/被试间变量). 如果是被试内变量, 需要选择要进行绘图的变量; 如果是被试间变量, 需要选择要进行检验的数据变量 (Y轴数据) 和分组变量 (X轴上的不同值). 点击 `生成` 按钮即可生成图片, 生成后, 点击 `保存图片` 按钮可以保存图片. 此外, 还可以自定义 X 轴和 Y 轴的标签',
		],
		[
		...
		[
			...
			'选择要进行绘图的变量, 并选择合适的词语过滤规则 (如 `非中文常见字` 会过滤掉除常见中文字外的所有字符), 也可自行输入过滤规则 (字词匹配或正则表达式, 如下图所示), 点击 `生成` 按钮即可生成图片, 生成后, 点击 `保存图片` 按钮可以保存图片. 此外, 还可以自定义词云图的颜色、词云图的形状、单词最大和最小尺寸、单词旋转角度、手动关闭词语切分等',
		],
		[
